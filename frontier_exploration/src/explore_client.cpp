    ...
    void pointCb(const geometry_msgs::PointStampedConstPtr& point){
        ...
        if(waiting_for_center_){
            ...
            if(!pointInPolygon(point->point,input_.polygon)){
                ...
                // send the goal_area to explore_server
...
public:
        ...
        waiting_for_center_(false)
    {
        ...
        double initial_goal_min = 0, initial_goal_max = 0;
        double wx_min, wy_min, wx_max, wy_max;
        private_nh_.param<bool>("initial_goal/flag", initial_goal_flag, true);
        if (initial_goal_flag) // Only applies to ideal cases
        {
            private_nh_.param<double>("initial_goal/bl", initial_goal_min, -1);
            private_nh_.param<double>("initial_goal/tr", initial_goal_max, 19);
            ...
            geometry_msgs::PointStamped point_center;
            point_center.point.x = point_center.point.y = 0;
            point_center.header.frame_id = map_frame_;
            exploreClient.waitForServer();
            ROS_INFO("Sending goal");
            frontier_exploration::ExploreTaskGoal goal;
            goal.explore_center = point_center;
            goal.explore_boundary = input_;
            exploreClient.sendGoal(goal);
        }
    }    
