...
public:
    ...
    FrontierExplorationServer(std::string name) :
        ...
        move_client_("move_base",true), // should change to robot_1/move_base
        retry_(5)
    {
        private_nh_.param<double>("frequency", frequency_, 0.0);
        private_nh_.param<double>("goal_aliasing", goal_aliasing_, 0.0);
        ...
        air_goal_pub_ = nh_.advertise<geometry_msgs::PointStamped>("robot_0/goal_location",5);
        air_pose_sub_ = nh_.subscribe("robot_0/odom",5,&FrontierExplorationServer::airOdomCallback,this);
    }
...
private:
    ...
    ros::Publisher air_goal_pub_;
    ros::Subscriber air_pose_sub_;
    geometry_msgs::PoseStamped air_pose_;

    void airOdomCallback(const nav_msgs::Odometry& odom){
        air_pose_.pose  = odom.pose.pose;
        air_pose_.header = odom.header;
    }
    ...
    void executeCb(const frontier_exploration::ExploreTaskGoalConstPtr &goal)
    {
        ...
        ros::ServiceClient getNextFrontier = private_nh_.serviceClient<frontier_exploration::GetNextFrontier>("explore_costmap/air_ground_explore/get_next_frontier");
        ...
        while(ros::ok() && as_.isActive()){
            ...
            //placeholder for next goals to be sent to move base and drone
            geometry_msgs::PoseStamped goal_pose_ground;
            geometry_msgs::PointStamped goal_point_air;

            //get current robot pose in frame of exploration boundary
            tf::Stamped<tf::Pose> ground_pose; // data format different from PoseStamped
            explore_costmap_ros_->getRobotPose(ground_pose);

            //provide current robot pose to the frontier search service request
            tf::poseStampedTFToMsg(ground_pose,srv.request.start_pose_ground);
            srv.request.start_pose_air = air_pose_;


            //evaluate if robot is within exploration boundary using robot_pose in boundary frame
            geometry_msgs::PoseStamped eval_pose = srv.request.start_pose_ground;
            //if(eval_pose.header.frame_id != goal->explore_boundary.header.frame_id){
            //    tf_listener_.transformPose(goal->explore_boundary.header.frame_id, srv.request.start_pose_ground, eval_pose);// transform pose into costmap frame
            //}
            ...
            if (false){
                ...
                if(success_){
                    ...
                    ROS_WARN("Robot not initially in exploration boundary, traveling to center");
                }
                ...
                goal_pose_ground.header = goal->explore_center.header;
                goal_pose_ground.pose.position = goal->explore_center.point;
                goal_pose_ground.pose.orientation = tf::createQuaternionMsgFromYaw( yawOfVector(eval_point.point, goal->explore_center.point) );

            }else if(getNextAirGroundFrontier.call(srv)){ //if in boundary, try to find next frontier to search
                ROS_DEBUG("Found frontier to explore");
                success_ = true;
                goal_pose_ground = feedback_.next_frontier = srv.response.next_frontier_ground;
                goal_point_air.point = srv.response.next_frontier_air.pose.position;
                goal_point_air.header = srv.response.next_frontier_air.header;
                retry_ = 5;
            ...
            //if(!pointsNearby(move_client_goal_.target_pose.pose.position,goal_pose_ground.pose.position,goal_aliasing_*0.5)){
            if (true){ // substitute goal aliasing
                //ROS_INFO("New exploration goal");
                move_client_goal_.target_pose = goal_pose_ground;
                boost::unique_lock<boost::mutex> lock(move_client_lock_);
                if(as_.isActive()){
            ...
            //send goal to uav
            air_goal_pub_.publish(goal_point_air);
    ...
    void doneMovingCb(const actionlib::SimpleClientGoalState& state, const move_base_msgs::MoveBaseResultConstPtr& result){
        ...
        if (state == actionlib::SimpleClientGoalState::ABORTED){
            ...
            ros::ServiceClient blacklistPointService = private_nh_.serviceClient<BlacklistPoint>("explore_costmap/air_ground_explore/blacklist_point");
            ...
            ros::ServiceClient clearBlacklistService = private_nh_.serviceClient<std_srvs::Empty>("explore_costmap/air_ground_explore/clear_blacklist");
