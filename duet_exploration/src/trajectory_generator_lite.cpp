...
MatrixXd TrajectoryGenerator::BezierPloyCoeffGeneration2D(
...
#define MINORDER  minimize_order
    ...
    Vector2d StartPt  = pos.row(0); 
    Vector2d EndPt    = pos.row(1); 

    Vector2d StartVel = vel.row(0);
    Vector2d EndVel   = vel.row(1);

    Vector2d StartAcc = acc.row(0);
    Vector2d EndAcc   = acc.row(1);
    
    int n_poly = traj_order + 1;
    int _s1d1CtrlP_num = n_poly;
    int _s1CtrlP_num   = 2 * _s1d1CtrlP_num;

    int equ_con_s_num = 3 * 2; // p, v, a in x, y axis at the start point
    int equ_con_e_num = 3 * 2; // p, v, a in x, y axis at the end point
    int equ_con_continuity_num = 3 * 2 * (_segment_num - 1);
    int _equ_con_num   = equ_con_s_num + equ_con_e_num + equ_con_continuity_num; // p, v, a in x, y in each segment's joint position
    
    int _high_order_con_num = 2 * traj_order * _segment_num;
    int _CtrlP_num          = _segment_num * _s1CtrlP_num;
    
    int con_num = _equ_con_num + _high_order_con_num;
    //cout<<"all constraints number is: "<<con_num<<endl;
    ...
    // for acceleration constraints
    /*for(int k =0; k < _segment_num; k++ ){
        for(int p = 0; p < 3; p++) // loop for x, y
            for(int i = 0; i < _s1d1CtrlP_num - 2; i++){
                pair<MSKboundkeye, pair<double, double> > cb_ie = make_pair( MSK_BK_RA, make_pair( - maxAcc,  maxAcc) ); 
                con_bdk.push_back(cb_ie);   
            }
    ...
    for(int i = 0; i < _equ_con_num; i ++ ){ 
        double beq_i;
        if(i < 2)                    beq_i = StartPt(i); 
        else if (i >= 2  && i < 4  ) beq_i = StartVel(i - 2); 
        else if (i >= 4  && i < 6  ) beq_i = StartAcc(i - 4);
        else if (i >= 6  && i < 8 ) beq_i = EndPt (i - 6 );
        else if (i >= 8  && i < 10 ) beq_i = EndVel(i - 8);
        else if (i >= 10 && i < 12 ) beq_i = EndAcc(i - 10);
        else beq_i = 0.0;
...
/*    row_idx = row_idx_shift;
    ...
    {   
        ...
        {  // loop for x, y      
            ...
            asub[0] = _CtrlP_num - 1 - (1 - i) * _s1d1CtrlP_num;
            aval[0] = 1.0 * lstScale;

            //cout<<" position idx: "<<asub[0]<<endl;
            r = MSK_putarow(task, row_idx, nzi, asub, aval);    
            row_idx ++;
        }
        // velocity :
        for(int i = 0; i < 2; i++)
        {
            ...
            MSKint32t asub[nzi];
            double aval[nzi];
            asub[0] = _CtrlP_num - 1 - (1 - i) * _s1d1CtrlP_num - 1;
            asub[1] = _CtrlP_num - 1 - (1 - i) * _s1d1CtrlP_num;
            aval[0] = - 1.0;
            aval[1] =   1.0;
            
            //cout<<" velocity idx: "<<asub[0]<<endl;
            //cout<<" velocity idx: "<<asub[1]<<endl;
            r = MSK_putarow(task, row_idx, nzi, asub, aval);    
            row_idx ++;
        }
        // acceleration : 
        for(int i = 0; i < 2; i++)
        {
            ...
            MSKint32t asub[nzi];
            double aval[nzi];
            asub[0] = _CtrlP_num - 1 - (1 - i) * _s1d1CtrlP_num - 2;
            asub[1] = _CtrlP_num - 1 - (1 - i) * _s1d1CtrlP_num - 1;
            asub[2] = _CtrlP_num - 1 - (1 - i) * _s1d1CtrlP_num;
            aval[0] =   1.0 / lstScale;
            aval[1] = - 2.0 / lstScale;
            aval[2] =   1.0 / lstScale;

            //cout<<" acceleration idx: "<<asub[0]<<endl;
            //cout<<" acceleration idx: "<<asub[1]<<endl;
            //cout<<" acceleration idx: "<<asub[2]<<endl;
            r = MSK_putarow(task, row_idx, nzi, asub, aval);    
            row_idx ++;
        }
    }
    ...
    {   
        int NUMQ_blk = (traj_order + 1);                       // default minimize the jerk and MINORDER = 3
        NUMQNZ      += 2 * NUMQ_blk * (NUMQ_blk + 1) / 2;
    }
    MSKint32t  qsubi[NUMQNZ], qsubj[NUMQNZ];
    double     qval[NUMQNZ];
    ...
    PolyCoeff = MatrixXd::Zero(_segment_num, 2 *(traj_order + 1) );

    int var_shift = 0;
    for(int i = 0; i < _segment_num; i++ )
    {
        for(int j = 0; j < 2 * n_poly; j++)
            PolyCoeff(i , j) = d_var(j + var_shift);
