...
public:
    ...
    {   
        ...
        _cmd_odom_pub = 
            handle.advertise<nav_msgs::Odometry>("odom_position_command", 50);

        _cmd_vel_pub = 
            handle.advertise<geometry_msgs::Twist>("cmd_vel", 3);
    ...
    void rcvOdometryCallback(const nav_msgs::Odometry & odom)
    {
        ...
        _vis_cmd.header.frame_id = "/robot_1/odom";

        if(state == INIT) //&& fabs(_odom.pose.pose.position.z  - 0.0) < 0.1 )
            cmd_flag = true;
        ...
        if(state == INIT )
        {
            ...
            _cmd.header.frame_id = "/robot_1/odom";
            ...
            _cmd_odom.header= _cmd.header;
            _cmd_odom.pose.pose.position= _cmd.position;
            _cmd_odom.twist.twist.linear= _cmd.velocity;
            _cmd_odom.pose.pose.orientation.w=1;
            _cmd_odom.pose.pose.orientation.x=0;
            _cmd_odom.pose.pose.orientation.y=0;
            _cmd_odom.pose.pose.orientation.z=0;

            _cmd_vel.linear= _cmd.velocity;
            _cmd_vel.angular.z=0;
            _cmd_vel_pub.publish(_cmd_vel);

            _cmd_odom_pub.publish(_cmd_odom);
...
/*        ROS_WARN("In trajectory server");
    ...
    void pubPositionCommand()
    {
        ...
        if (state == HOVER)
        {
            if (_cmd.header.frame_id != "/robot_1/odom"){
                _cmd.position = _odom.pose.pose.position;
            }
...
/*                        cout<<"C:\n"<<CList[cur_order]<<endl;
        ...
        _vis_vel.ns = "vel";
        _vis_vel.id = 0;
        _vis_vel.header.frame_id = "/robot_1/odom";
        ...
        _vis_acc.ns = "acc";
        _vis_acc.id = 0;
        _vis_acc.header.frame_id = "/robot_1/odom";
